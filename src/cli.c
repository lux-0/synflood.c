 ...
 */
bool validateHostname(const char *inp, char hostname[HOSTNAME_BUFFER_LENGTH], unsigned short int port,
 ...
 */
void getOptions(int argc, char *argv[], char hostname[HOSTNAME_BUFFER_LENGTH], unsigned short int *port,
                struct sockaddr_in *host_addr, unsigned int *attack_time)

{
    int opt = 0;
    char *hostname_placeholder;
    bool hostname_initialized = false, port_initialized = false, attack_time_intialized = false;

    *attack_time = DEFAULT_ATTACK_TIME;

    struct option option_array[] = {{"help", no_argument, NULL, (int)'H'},
